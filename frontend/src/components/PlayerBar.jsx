...
export default function PlayerBar() {
  ...
  if (!currentTrack) {
    return (
      <div className="h-[70px] lg:h-[90px] bg-elevated border-t border-border flex items-center justify-center text-text-muted text-[0.8rem] lg:text-[0.85rem] px-4 text-center">
  ...
  return (
    <div className="h-[70px] lg:h-[90px] bg-elevated border-t border-border grid grid-cols-[1fr_auto] lg:grid-cols-3 items-center px-3 lg:px-4 gap-2 lg:gap-4">
      {}
      <div className="flex items-center gap-2.5 lg:gap-3 min-w-0">
        <img src={currentTrack.artworkUrl || '/placeholder-cover.svg'} alt="" className="w-11 h-11 lg:w-[54px] lg:h-[54px] rounded-sm object-cover bg-elevated-2 flex-shrink-0" />
        <div className="flex flex-col gap-0.5 min-w-0">
          <span className="truncate text-[0.82rem] lg:text-[0.88rem] font-semibold">{currentTrack.title}</span>
          <span className="truncate text-[0.72rem] lg:text-[0.78rem] text-text-secondary">{currentTrack.artistName}</span>
        </div>
        {isAuthenticated && (
          <button
